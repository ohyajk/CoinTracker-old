    ...
    <div className="">
      <div className="w-[100%] flex justify-center items-center">
        <form className="text-white flex items-center justify-center w-[90%] md:w-[60%] m-2">
          <input onKeyUp={(e) => {setQuery(e.target.value)}} className="bg-[#0d0b13] px-4 py-2 outline-none w-[80%] border border-white/50 rounded-l-full focus:border-violet-600/50 focus:shadow-sm shadow-violet-400" type="text" placeholder="Search Coin name" />
      ...
      <div>
        ...
        <ul className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {allCoins?.filter((coin) => {return coin.name.toLowerCase().includes(query.toLowerCase())}).map((coinf) => (
