  ...
  return (
    <div className="">
      ...
      <ul>
        ...
        {allCoins?.map((coin) => (
          <Link to="/coin" onClick={() => { dispatch(addCoin(coin)); }} key={coin.id}>
            <li className="flex items-center gap-2 py-2 px-4 bg-violet-900/20 m-2">
              ...
              <span className={coin.priceChange1d < 0 ? 'text-red-400 ml-auto' : 'text-green-400 ml-auto'}>
                {coin.priceChange1d}
                {' '}
                <i className={coin.priceChange1d < 0 ? 'fa-solid fa-arrow-trend-down' : 'fa-solid fa-arrow-trend-up'} />
              </span>
            </li>
          </Link>
        ))}
